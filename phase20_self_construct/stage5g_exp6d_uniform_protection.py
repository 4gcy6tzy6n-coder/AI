"""
Stage 5G - Experiment 6D: Uniform Step Protection

实验 6D：统一步骤保护

核心改进：
1. 所有步骤使用统一更新上限 0.003
2. 保持高基础 KL：0.32/0.40
3. 降低学习率到 1.0e-5 提高稳定性
...
class Exp6DConfig:
    ...
    learning_rate: float = 1.0e-5  # 降低提高稳定性
    num_epochs: int = 3
    batch_size: int = 8
    replay_ratio: float = 0.35
    # 统一更新上限
    max_change: float = 0.003  # 所有步骤统一
...
class UniformProtectionPromotion:
    def __init__(self, model_path: str, config: Exp6DConfig):
        self.config = config
    ...
    def guided_promotion_step(self, new_samples: List[FinetuneSample],
                              ...
                              step_num: int) -> Dict:
        self.model.train()
        max_change = self.config.max_change  # 统一使用
        epoch_results = []
...
class Experiment6D:
    ...
    def run_continuous_promotion(self, candidates: List[Dict]) -> Dict:
        ...
        promoter = UniformProtectionPromotion(self.model_path, self.config)
    ...
    def run(self):
        ...
        if all_passed:
            ...
            print("\n分析:")
            print("  连续多轮实验显示核心矛盾:")
            print("  - 第 1 步保护和学习能力可以平衡")
            print("  - 但连续多步后旧能力会累积损伤")
            print("\n建议调整阶段 6 入口标准:")
            print("  - 接受'第 1 步 < 8%'作为核心指标")
            print("  - 放宽'最终 < 10%'到'最终 < 12%'")
            print("  - 或改为'每步后都 < 10%'")

        results = {
            'experiment': 'exp6d_uniform_protection',
            'config': {
                'base_kl_weights': self.config.base_kl_weights,
                'learning_rate': self.config.learning_rate,
                'replay_ratio': self.config.replay_ratio,
                'max_change': self.config.max_change,
            },
            'continuous_result': continuous_result,
            'phase6_ready': all_passed,
        }

        with open("eval/stage5g_exp6d_results.json", 'w') as f:
            json.dump(results, f, indent=2)

        print("\n✓ 实验 6D 完成")
        print("  结果已保存到 eval/stage5g_exp6d_results.json")
        return results


def main():
    model_path = "../phase19_native_backbone/checkpoints/native_128.pt"
    experiment = Experiment6D(model_path)
