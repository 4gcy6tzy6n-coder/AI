"""
Stage 8-R2 Curriculum + Dynamic Balance
课程学习 + 动态平衡
...
- 阶段3: L1 + L2 + L3 (75步)
- L1 权重: 1.3 (从 2.0 降低)
- L1 replay: 每 20 步 (从 10 步减少)
- 固定混合采样: L1:L2:L3 = 2:2:1
...
class Stage8R2CurriculumBalance:
    ...
    def __init__(
        ...
        # R2 策略参数
        self.l1_weight = 1.3  # 从 2.0 降低
        self.replay_interval = 20  # 从 10 增加到 20
        self.sampling_ratio = {'L1': 2, 'L2': 2, 'L3': 1}  # 固定比例 2:2:1
    ...
    def establish_baseline(self):
        ...
        print(f"[R2 Balance] Writeback 基线: {self.baseline_wb_score:.4f}")
    ...
    def get_available_datasets(self, phase: str) -> List[Tuple[List[Dict], str]]:
        """获取当前阶段可用的数据集"""
        if phase == 'L1_warmup':
            return [(self.l1_dataset, 'L1')]
        elif phase == 'L1_L2':
            return [(self.l1_dataset, 'L1'), (self.l2_dataset, 'L2')]
        else:  # L1_L2_L3
            return [(self.l1_dataset, 'L1'), (self.l2_dataset, 'L2'), (self.l3_dataset, 'L3')]
    ...
    def compute_composite_score(self, l1_acc: float, l2_acc: float, l3_acc: float, wb_change: float) -> float:
        """
        计算综合评分
        Score = 0.35 * L1 + 0.35 * L2 + 0.20 * L3 + 0.10 * WB
        WB = max(0, 1 - |writeback_change| / 0.05)  # 稳定性奖励
        """
        wb_score = max(0, 1 - abs(wb_change) / 0.05)
        return 0.35 * l1_acc + 0.35 * l2_acc + 0.20 * l3_acc + 0.10 * wb_score
    
    def train_step(self, step: int) -> Tuple[BalanceMetrics, float]:
        """执行一步训练"""
        self.model.train()
        ...
        # 2. L1 Replay (每 20 步，且只在非 L1_warmup 阶段)
    ...
    def train(self, num_steps: int = 200) -> List[BalanceMetrics]:
        """执行课程学习训练"""
        print("\n" + "="*70)
        print("Stage 8-R2 Curriculum + Dynamic Balance")
        print("="*70)
        ...
        print(f"  Phase 2 (50-124): L1 + L2")
        print(f"  Phase 3 (125-199): L1 + L2 + L3")
        print(f"\n修复策略:")
        print(f"  - L1 权重: {self.l1_weight}")
        print(f"  - L1 replay: 每 {self.replay_interval} 步")
        print(f"  - 采样比例: L1:L2:L3 = {self.sampling_ratio['L1']}:{self.sampling_ratio['L2']}:{self.sampling_ratio['L3']}")
    ...
    def generate_report(self) -> Dict:
        ...
        # 检查跷跷板效应 (L1 和 L2 是否同时达标)
        seesaw_ok = final.l1_accuracy > 0.60 and final.l2_accuracy > 0.40
        ...
        report = {
            'experiment_id': 'stage8_r2_curriculum_balance',
            'timestamp': datetime.now().isoformat(),
            'config': {
            ...
            'acceptance': {
                ...
                'seesaw_check': {'value': seesaw_ok, 'target': 'L1>60% & L2>40%', 'pass': seesaw_ok},
                'all_pass': all_pass,
            },
...
def run_stage8_r2():
    ...
    # 3. 创建训练器
    trainer = Stage8R2CurriculumBalance(
        model=model,
    ...
    # 4. 执行训练
    metrics = trainer.train(num_steps=200)
    
    # 5. 生成报告
    report = trainer.generate_report()
    
    # 6. 保存报告
    report_path = Path('stage8_dataset/r2_curriculum_report.json')
    with open(report_path, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    
    # 7. 打印结果
    print("\n" + "="*70)
    print("Stage 8-R2 Curriculum + Dynamic Balance 报告")
    print("="*70)
    ...
    if acceptance['all_pass']:
        print("✓✓✓ STAGE 8-R2 CURRICULUM + BALANCE 通过 ✓✓✓")
    else:
        print("✗✗✗ STAGE 8-R2 CURRICULUM + BALANCE 未通过 ✗✗✗")
    print("="*70)
