"""
Stage 5G - Experiment 6B: Stronger Step 1 Protection

实验 6B：更强第 1 步保护

核心改进：
1. 进一步提高基础 KL：gap/policy/governance=0.30, writeback=0.38
2. 收紧第 1 步更新幅度上限到 0.003
3. 增加第 1 步前的 replay warm-up

目标：把第 1 步旧能力掉落压到 < 8%

配置：
- base_kl = {"gap": 0.30, "policy": 0.30, "governance": 0.30, "writeback": 0.38}
- learning_rate = 1.0e-5
- replay_ratio = 0.35
- step1_max_change = 0.003  # 更严格
- step2_max_change = 0.008
- kl_threshold_high = 0.04  # 更早触发
"""
...
class StrongerStep1Promotion:
    """更强第 1 步保护参数晋升"""

    def __init__(self, model_path: str, config: Exp6BConfig):
        self.config = config
    ...
    def compute_layered_kl(self, current_outputs: Dict) -> torch.Tensor:
        ...
        for logits_key, config_key in key_mapping.items():
            if logits_key in current_outputs and logits_key in self.baseline_outputs:
                ...
                weight = self.current_kl_weights.get(config_key, 0.30)
                kl_loss += kl * weight
    ...
    def run_warmup(self, replay_samples: List[FinetuneSample]):
        """Warm-up：只用旧能力样本稳定模型"""
        self.model.train()

        print("  Warm-up: 稳定旧能力...")
        for epoch in range(self.config.warmup_epochs):
            for sample in replay_samples[:20]:  # 只用部分样本
                ...
                total_loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), 0.002)
                self.optimizer.step()
    ...
    def guided_promotion_step(self, new_samples: List[FinetuneSample],
        ...
        # 根据步数选择更新幅度上限
        if step_num == 1:
            max_change = self.config.step1_max_change
        else:
            max_change = self.config.step2_max_change
...
class Experiment6B:
    ...
    def run_continuous_promotion(self, candidates: List[Dict]) -> Dict:
        ...
        promoter = StrongerStep1Promotion(self.model_path, self.config)
        ...
        # 第 1 步前 warm-up
        print("\n--- Warm-up 阶段 ---")
        promoter.run_warmup(replay_samples)
        warmup_eval = promoter.evaluate_detailed()
        print(f"Warm-up 后旧能力掉落: {warmup_eval['max_old_drop']:.2%}")
    ...
    def run(self):
        """运行实验 6B"""
        print("=" * 70)
        print("Stage 5G - 实验 6B: 更强第 1 步保护")
        print("=" * 70)
        ...
        print(f"  自适应阈值: 高>{self.config.kl_threshold_high}")
        print(f"  Warm-up: {self.config.warmup_epochs} epoch")
        ...
        # 保存结果
        results = {
            'experiment': 'exp6b_stronger_protection',
            'config': {
        ...
        with open("eval/stage5g_exp6b_results.json", 'w') as f:
            json.dump(results, f, indent=2)

        print("\n✓ 实验 6B 完成")
        print("  结果已保存到 eval/stage5g_exp6b_results.json")
...
def main():
    ...
    experiment = Experiment6B(model_path)
