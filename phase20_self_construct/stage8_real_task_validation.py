...
class Stage8RealTaskValidator:
    ...
    def establish_baseline(self):
        ...
        print(f"[Real Task] Writeback 基线: {self.baseline_wb_score:.4f}")
    ...
    def train_step(self, step: int) -> Tuple[ValidationMetrics, float]:
        """执行一步训练"""
        self.model.train()
        ...
        if step % 3 == 0:
            ...
            dataset = self.l2_dataset
            difficulty = 'L2'
        else:
            dataset = self.l3_dataset
            difficulty = 'L3'
    ...
    def validate(self, num_steps: int = 200) -> List[ValidationMetrics]:
        """执行验证"""
        print("\n" + "="*70)
        print("Stage 8 真实任务升级验证")
        print("="*70)
    ...
    def generate_report(self) -> Dict:
        ...
        report = {
            'experiment_id': 'stage8_real_task',
            'timestamp': datetime.now().isoformat(),
            'config': {
...
def run_stage8_real_task_validation():
    """运行真实任务验证"""
    print("="*70)
    print("Stage 8 真实任务升级验证")
    print("="*70)
    ...
    # L1 数据
    l1_path = Path('stage8_dataset/train.jsonl')
    if l1_path.exists():
        l1_dataset = load_jsonl(str(l1_path))
    else:
        print(f"✗ L1 数据集不存在: {l1_path}")
        return
    
    # L2 数据
    l2_path = Path('stage8_dataset/synthetic_l2.jsonl')
    if l2_path.exists():
        l2_dataset = load_jsonl(str(l2_path))
    else:
        print(f"✗ L2 数据集不存在: {l2_path}")
        return
    
    # L3 数据
    l3_path = Path('stage8_dataset/synthetic_l3.jsonl')
    if l3_path.exists():
        l3_dataset = load_jsonl(str(l3_path))
    else:
        print(f"✗ L3 数据集不存在: {l3_path}")
        return
    ...
    # 3. 创建验证器
    validator = Stage8RealTaskValidator(
        model=model,
    ...
    # 4. 执行验证
    metrics = validator.validate(num_steps=200)
    
    # 5. 生成报告
    report = validator.generate_report()
    
    # 6. 保存报告
    report_path = Path('stage8_dataset/real_task_report.json')
    with open(report_path, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    
    # 7. 打印结果
    print("\n" + "="*70)
    print("Stage 8 真实任务升级验证报告")
    print("="*70)
    ...
    if acceptance['all_pass']:
        print("✓✓✓ STAGE 8 真实任务升级验证通过 ✓✓✓")
    else:
        print("✗✗✗ STAGE 8 真实任务升级验证未通过 ✗✗✗")
    print("="*70)
...
if __name__ == "__main__":
    report = run_stage8_real_task_validation()
