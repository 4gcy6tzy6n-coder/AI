"""
Stage 10-2R1: 真实数据迁移修正验证

目标:
- 验证真实数据下系统能否在更长训练中形成持续平衡窗口
- 降低真实数据比例 (30% → 15%)，减少分布冲击
- 延长训练步数 (500 → 800)，给系统更多适应时间
...
class Stage10R1RefinedValidator:
    """Stage 10-2R1 修正验证器"""
    ...
    def __init__(
        ...
        # 记录
        self.metrics_history: List[Stage10R1Metrics] = []
    ...
    def establish_baseline(self):
        ...
        print(f"[S10-2R1] Writeback 基线: {self.baseline_wb_score:.4f}")
    ...
    def train_step(self, step: int) -> Tuple[Optional[Stage10R1Metrics], float]:
        """执行一步训练"""
        self.model.train()
    ...
    def train(self, num_steps: int = 800) -> List[Stage10R1Metrics]:
        """执行训练"""
        print("\n" + "="*70)
        print("Stage 10-2R1: 真实数据迁移修正验证")
        print("="*70)
        print(f"合成数据:")
        ...
        print(f"\n关键调整 (vs S10-2):")
        print(f"  - 真实数据比例: {self.real_data_ratio*100:.0f}% (原为 30%)")
        print(f"  - 训练步数: {num_steps} (原为 500)")
        print(f"  - 其他参数: 保持 R2 基线冻结")
        print(f"\n课程阶段:")
        print(f"  Phase 1 (0-49): L1 warm-up")
        print(f"  Phase 2 (50-119): L1 + L2")
        print(f"  Phase 3 (120-799): L1 + L2 + L3 (真实数据混合)")
        print(f"\n观察重点:")
        print(f"  - Step 500 之后是否出现持续平衡窗口")
        print(f"  - L1 是否重新恢复并稳定")
        print(f"  - Writeback 是否继续保持极低漂移")
        print("="*70)
        ...
        for step in range(num_steps):
            ...
            if metrics is not None:
                ...
                milestone = ""
                if step == 500:
                    milestone = "【Step 500 - 关键观察点】"
                elif step > 500 and metrics.balance_window_active:
                    milestone = "【500+ 平衡出现】"
                
                print(f"\n[Step {step+1}/800 | {metrics.phase}] {balance_marker} {source_marker} {milestone}")
    ...
    def generate_report(self) -> Dict:
        ...
        # 关键: Step 500 后的平衡窗口
        late_metrics = [m for m in self.metrics_history if m.step >= 500]
        late_balance_window = 0
        if late_metrics:
            late_balanced = [m for m in late_metrics if m.balance_window_active]
            if late_balanced:
                late_balance_window = len(late_balanced) * 50  # 每 50 steps 记录一次
        
        balance_window_pass = self.max_balance_window >= 50
        late_balance_pass = late_balance_window >= 100  # Step 500 后至少 100 steps 平衡
        ...
        report = {
            'experiment_id': 'stage10_2r1_real_data_refined',
            'timestamp': datetime.now().isoformat(),
            'config': {
                'num_steps': 800,
            ...
            'stability_analysis': {
                ...
                'late_balance_window': late_balance_window,
            ...
            'acceptance': {
                'coexistence': {'value': coexistence, 'pass': coexistence},
                'writeback_stable': {'value': wb_stable, 'pass': wb_stable},
                'balance_window': {'value': self.max_balance_window, 'pass': balance_window_pass},
                'late_balance_window': {'value': late_balance_window, 'pass': late_balance_pass},
                'all_pass': all_pass,
            },
...
def run_stage10_2r1():
    ...
    # 4. 创建训练器
    trainer = Stage10R1RefinedValidator(
        model=model,
    ...
    # 5. 执行训练
    metrics = trainer.train(num_steps=800)
    
    # 6. 生成报告
    report = trainer.generate_report()
    
    # 7. 保存报告
    report_path = Path('stage8_dataset/s10_2r1_refined_report.json')
    with open(report_path, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    
    # 8. 打印结果
    print("\n" + "="*70)
    print("Stage 10-2R1 修正验证报告")
    print("="*70)
    ...
    print(f"\n[关键观察 - Step 500 后]")
    print(f"  后期平衡窗口: {stability['late_balance_window']} steps")
    
    print(f"\n[验收检查]")
    print(f"  多层共存: {'✓' if acceptance['coexistence']['pass'] else '✗'}")
    print(f"  Writeback 稳定: {'✓' if acceptance['writeback_stable']['pass'] else '✗'}")
    print(f"  总平衡窗口: {stability['max_balance_window']} steps")
    print(f"  后期平衡窗口 ≥ 100: {'✓' if acceptance['late_balance_window']['pass'] else '✗'}")
    
    print("\n" + "="*70)
    if acceptance['all_pass']:
        print("✓✓✓ STAGE 10-2R1 修正验证通过 ✓✓✓")
        print("\n🎉 真实数据下持续平衡窗口形成！")
    else:
        print("✗✗✗ STAGE 10-2R1 修正验证未通过 ✗✗✗")
    print("="*70)
