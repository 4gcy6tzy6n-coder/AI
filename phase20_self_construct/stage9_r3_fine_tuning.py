...
策略:
- L1 权重: 1.2 → 1.3 (轻微提升)
- 保持固定采样: L1:L2:L3 = 2:2:1
- L1 replay: 每 10 步 → 每 8 步 (小幅增强)
- Phase 保持: 1-50 L1, 51-120 L1+L2, 121+ 全混合

目标: L1 ≥ 50%, 保持 100-step 平衡窗口
"""
...
class Stage9R3FineTuningTrainer:
    """Stage 9-R3 微调训练器"""
    ...
    def __init__(
        ...
        # 记录
        self.metrics_history: List[Stage9R3Metrics] = []
        ...
        # Replay 配置 (更频繁)
        self.replay_interval = 8  # 从 10 改为 8
    ...
    def establish_baseline(self):
        ...
        print(f"[S9-R3] Writeback 基线: {self.baseline_wb_score:.4f}")
    ...
    def train_step(self, step: int) -> Tuple[Optional[Stage9R3Metrics], float]:
        """执行一步训练"""
        self.model.train()
    ...
    def train(self, num_steps: int = 300) -> List[Stage9R3Metrics]:
        """执行训练"""
        print("\n" + "="*70)
        print("Stage 9-R3: 微调收口")
        print("="*70)
        ...
        print(f"\n微调策略:")
        print(f"  - 固定权重: L1=1.3, L2=1.2, L3=1.0")
        print(f"  - 采样比例: L1:L2:L3 = 2:2:1")
        print(f"  - L1 replay: 每 {self.replay_interval} 步 (增强)")
        print(f"\n课程阶段:")
        print(f"  Phase 1 (0-49): L1 warm-up")
        print(f"  Phase 2 (50-119): L1 + L2")
        print(f"  Phase 3 (120-299): L1 + L2 + L3")
        print(f"\n验收标准 (R3):")
        print(f"  - L1 ≥ 50%, L2 ≥ 75%, L3 ≥ 60%")
        print(f"  - Overall ≥ 60%")
        print(f"  - Balance Window ≥ 100 steps")
        print(f"  - L1 不清零")
    ...
    def generate_report(self) -> Dict:
        ...
        # R3 验收标准 (更严格)
        l1_pass = final.l1_accuracy >= 0.50  # 关键提升
        l2_pass = final.l2_accuracy >= 0.75
        l3_pass = final.l3_accuracy >= 0.60
        overall_pass = final.overall_accuracy >= 0.60
        wb_pass = abs(final.writeback_change) < 0.05
        window_pass = self.max_balance_window >= 100
        
        l1_dropped_to_zero = any(m.l1_accuracy < 0.05 for m in self.metrics_history)
        
        all_pass = l1_pass and l2_pass and l3_pass and overall_pass and wb_pass and window_pass and not l1_dropped_to_zero
        
        tsla_summary = self.tsla_gate.get_event_summary()
        
        report = {
            'experiment_id': 'stage9_r3_fine_tuning',
            'timestamp': datetime.now().isoformat(),
            'config': {
                'num_steps': 300,
            ...
            'acceptance': {
                ...
                'l2_accuracy': {'value': final.l2_accuracy, 'target': '≥ 75%', 'pass': l2_pass},
                'l3_accuracy': {'value': final.l3_accuracy, 'target': '≥ 60%', 'pass': l3_pass},
                ...
                'balance_window': {'value': self.max_balance_window, 'target': '>= 100 steps', 'pass': window_pass},
...
def run_stage9_r3():
    ...
    # 3. 创建训练器
    trainer = Stage9R3FineTuningTrainer(
        model=model,
    ...
    # 6. 保存报告
    report_path = Path('stage8_dataset/s9_r3_fine_tuning_report.json')
    with open(report_path, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    
    # 7. 打印结果
    print("\n" + "="*70)
    print("Stage 9-R3 微调收口实验报告")
    print("="*70)
    ...
    if acceptance['all_pass']:
        print("✓✓✓ STAGE 9-R3 微调收口实验通过 ✓✓✓")
        print("\n🎉 单模型多层任务共存路线正式收口！")
        print("\n项目状态:")
        print("  - 主机制 (Guard + TSLA): 稳定")
        print("  - 多层任务共存: 验证通过")
        print("  - 系统整体: 可进入产品化阶段")
    else:
        print("✗✗✗ STAGE 9-R3 微调收口实验未通过 ✗✗✗")
        print("\n需要进一步微调或考虑其他策略")
    print("="*70)
