"""
Test Minimal Loop v4 - 最小闭环测试 v4
...
from phase15.orchestrator_llm_v4 import ConversationOrchestratorWithLLMV4
from phase15.llm_client_v1 import create_mock_client, create_deepseek_client


class MinimalLoopTestV4:
    """最小闭环测试 v4"""
    ...
    async def setup(self):
        ...
        if self.llm_provider == "deepseek":
            ...
            llm_client = create_deepseek_client()  # 从环境变量读取API key
        else:
            llm_client = create_mock_client()
        
        print(f"✓ 使用{self.llm_provider.upper()}客户端")
        
        # 健康检查
        health = await llm_client.health_check()
        if not health:
            raise RuntimeError(f"LLM客户端健康检查失败")
        
        print("✓ LLM客户端健康检查通过")
        
        # 创建Orchestrator v4
        self.orchestrator = ConversationOrchestratorWithLLMV4(
    ...
    async def run(self) -> dict:
        ...
        print("="*70)
        print("开始最小闭环测试 v4 - 检索优化验证")
        print("="*70)
        
        # 测试用例
        test_cases = [
            ("test_v4_001", "你好", "通用问候，无需检索"),
            ("test_v4_002", "我叫什么名字？", "应触发检索并回答Alice"),
            ("test_v4_003", "项目的目标是什么？", "应触发检索并回答项目目标"),
            ("test_v4_004", "技术栈是什么？", "应触发检索并回答技术栈"),
            ("test_v4_005", "我们之前讨论过什么？", "应触发检索并基于历史回答"),
        ]
...
async def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="最小闭环测试 v4")
