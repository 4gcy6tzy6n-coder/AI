"""
Orchestrator with Real LLM v1 - 集成真实LLM的对话主控层 v1

Phase 15 核心组件：
把 conversation_orchestrator_v1.py 里的模拟生成替换为真实模型调用
"""
    ...
    def build(
        ...
        if retrieval_results and hasattr(retrieval_results, 'results'):
            retrieval_context = [
                ...
                for r in retrieval_results.results[:3]
            ]
        
        # 转换治理上下文
        governance_context = GovernanceContext(
            strategy=governance_result.get('strategy', ResponseStrategy.DIRECT),
            confidence=governance_result.get('confidence', 0.7),
            risk_level=governance_result.get('risk_level', 'low'),
            requires_review=governance_result.get('strategy') == ResponseStrategy.REVIEW,
...
class ConversationOrchestratorWithLLM(BaseOrchestrator):
    """
    集成真实LLM的对话主控层
    
    继承自 BaseOrchestrator，替换响应生成为真实LLM调用
    """
    ...
    def _generate_response(
        ...
        response = OrchestratorResponse(
            ...
            metadata={
                ...
                "raw_strategy": llm_output.strategy.value,
                "governance_strategy": governance_result.get('strategy', 'UNKNOWN'),
                "strategy_aligned": validated_output.strategy == governance_result.get('strategy')
            },
    ...
    def _validate_and_calibrate(
        ...
        if gov_confidence < 0.6 and llm_output.strategy == ResponseStrategy.DIRECT:
    ...
    def _is_more_conservative(
        self,
        strategy1: ResponseStrategy,
        strategy2: ResponseStrategy
    ) -> bool:
        """判断strategy1是否比strategy2更保守"""
        conservativeness = {
            ResponseStrategy.DECLINE: 4,
            ResponseStrategy.REVIEW: 3,
            ResponseStrategy.CONSERVATIVE: 2,
            ResponseStrategy.RETRIEVAL_FIRST: 1,
            ResponseStrategy.DIRECT: 0
        }
        return conservativeness.get(strategy1, 0) > conservativeness.get(strategy2, 0)
    ...
    async def process_turn_async(
        ...
        print(f"  [完成] 总延迟: {response.metadata['total_latency_ms']:.0f}ms\n")
        
        return response


async def demo_with_llm():
    """演示集成LLM的Orchestrator"""
    print("="*70)
    print("Conversation Orchestrator with Real LLM - 演示")
    print("="*70)
    ...
    # 创建主控器
    orchestrator = ConversationOrchestratorWithLLM(
    ...
    test_memories = [
        ...
        ("long_term", "project_x", "项目 X 的目标是在 Q3 完成核心功能开发"),
    ]
